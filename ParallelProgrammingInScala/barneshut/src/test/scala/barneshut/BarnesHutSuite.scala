...
import FloatOps._
  ...
  // test cases for Body (must implement Body.traverse() first)
  ...
  // test cases for computeBoundaries

  test("updateBoundaries collapses the initial Boundaries to around a single Body") {
    val BHframe = BarnesHut.frame
    val simulator = BarnesHut.simulator
    val x: Float = 100f
    val y: Float = 200f
    val body = new Body(1000f, x, y, 30f, 0f)
    val boundaries = simulator.updateBoundaries(new Boundaries(), body)
    assert(boundaries.minX == x && boundaries.minY == y && boundaries.maxX == x && boundaries.maxY == y,
      s"Boundary should be ($x, $y, $x, $y) but is ${boundaries}")
  }

  test("updateBoundaries encompasses two bodies correctly.") {
    val BHframe = BarnesHut.frame
    val simulator = BarnesHut.simulator
    val x1: Float = 100f
    val y1: Float = 200f
    val body1 = new Body(1000f, x1, y1, 30f, 0f)
    val x2: Float = 400f
    val y2: Float = 0f
    val body2 = new Body(2000f, x2, y2, 0f, 0f)
    val boundaries = simulator.updateBoundaries(simulator.updateBoundaries(new Boundaries(), body1), body2)
    assert(boundaries.minX == x1 && boundaries.minY == y2 &&
      boundaries.maxX == x2 && boundaries.maxY == y1,
      s"Boundary should be (${x1}, ${y2}, ${x2}, ${y1}) but is ${boundaries}")
  }

  test("mergeBoundaries on a single Body is silly but still works.") {
    val BHframe = BarnesHut.frame
    val simulator = BarnesHut.simulator
    val x: Float = 100f
    val y: Float = 200f
    val body = new Body(1000f, x, y, 30f, 0f)
    val boundaries1 = simulator.updateBoundaries(new Boundaries(), body)
    val boundaries2 = simulator.updateBoundaries(new Boundaries(), body)    // Yes they overlap.
    val boundaries = simulator.mergeBoundaries(boundaries1, boundaries2)
    assert(boundaries.minX == x && boundaries.minY == y && boundaries.maxX == x && boundaries.maxY == y,
      s"Boundary should be ($x, $y, $x, $y) but is ${boundaries}")
  }

  test("mergeBoundaries encompasses two bodies correctly.") {
    val BHframe = BarnesHut.frame
    val simulator = BarnesHut.simulator
    val x1: Float = 100f
    val y1: Float = 200f
    ...
    val y2: Float = 0f
    val body2 = new Body(2000f, x2, y2, 0f, 0f)
    val boundaries1 = simulator.updateBoundaries(new Boundaries(), body1)
    val boundaries2 = simulator.updateBoundaries(new Boundaries(), body2)
    val boundaries = simulator.mergeBoundaries(boundaries1, boundaries2)
    assert(boundaries.minX == x1 && boundaries.minY == y2 &&
      boundaries.maxX == x2 && boundaries.maxY == y1,
      s"Boundary should be (${x1}, ${y2}, ${x2}, ${y1}) but is ${boundaries}")
